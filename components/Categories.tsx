      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-lg md:text-xl text-gray-medium">
            From birthdays to corporate events, find the perfect gift for any
            moment
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-6xl mx-auto">
          {categories.map((category, index) => (
            <div key={index} className="group cursor-pointer">
              <div
                className={`bg-gradient-to-br ${category.color} rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 h-full flex flex-col items-center text-center`}
              >
                {/* Icon */}
                <div
                  className={`${category.iconBg} w-16 h-16 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}
                >
                ...
                <p className="text-sm text-gray-dark">{category.description}</p>
              </div>
            </div>
          ))}
        </div>
